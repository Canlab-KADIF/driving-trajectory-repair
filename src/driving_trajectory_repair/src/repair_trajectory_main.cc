...
void PrintUsage(const char* program_name) {
  std::cerr
      ...
      << "Tuning parameters are node parameters; see "
         "config/driving_trajectory_repair.yaml\n"
...
void DeclareOptions(
    rclcpp::Node* node,
    keti::kadif::driving_trajectory_repair::BagRewriterInitOptions* out) {
...
int main(int argc, char** argv) {
  ...
  options.repaired_bag_uri =
      keti::kadif::driving_trajectory_repair::MakeOutputUri(
          options.input_bag_uri, suffix);
  options.interpolated_bag_uri =
      keti::kadif::driving_trajectory_repair::MakeOutputUri(
          options.input_bag_uri, suffix + "_interpolated_only");

  const auto node = std::make_shared<rclcpp::Node>("repair_trajectory");
