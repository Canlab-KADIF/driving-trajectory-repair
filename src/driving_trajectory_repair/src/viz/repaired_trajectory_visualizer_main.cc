...
class RepairedTrajectoryVisualizer : public rclcpp::Node {
 public:
  RepairedTrajectoryVisualizer();
...
RepairedTrajectoryVisualizer::RepairedTrajectoryVisualizer()
    : rclcpp::Node("repaired_trajectory_visualizer") {
  ...
  obstacle_subscription_ =
      create_subscription<cyber_perception_msgs::msg::PerceptionObstacles>(
          ...
          std::bind(&RepairedTrajectoryVisualizer::OnObstacles, this,
                    std::placeholders::_1));
...
int main(int argc, char** argv) {
  ...
  rclcpp::spin(std::make_shared<keti::kadif::driving_trajectory_repair::
                                    RepairedTrajectoryVisualizer>());
  rclcpp::shutdown();
  return 0;
}
